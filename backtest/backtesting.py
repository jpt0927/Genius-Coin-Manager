...
def leverage_backtest(strategy_function, strategy_param, leverage, progress_callback=None, plot_callback=None, initial_cash=100, window_size=pd.DateOffset(months=6),
                      step_size=pd.DateOffset(days=1), fee_rate=0.001):
    print(f"롤링 레버리지 백테스팅을 시작합니다. (레버리지: {leverage}x)")

    try:
        df = pd.read_csv('backtest/data.csv', parse_dates=['Open time'], index_col='Open time')
        df_hourly = df.resample('min').agg({
            'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'
        }).dropna()
        print("데이터 준비 완료")
    except FileNotFoundError:
    ...
    start_date = df_hourly.index[0]
    end_date = df_hourly.index[-1]
    current_start = start_date
    ...
    while current_start + window_size <= end_date:
        current_end = current_start + window_size
        window_df = df_hourly.loc[current_start:current_end]
        ...
        window_strategy_result = strategy_function(
            ...
            initial_cash=initial_cash,
            fee_rate=fee_rate,
            leverage=leverage  # leverage 파라미터 전달
        )
        ...
        all_results.append({
            ...
            'strategy_mdd': window_strategy_result['mdd_pct'],
            'total_trades': window_strategy_result['total_trades'],
            'total_liquidations': window_strategy_result['total_liquidations'] # 청산 횟수 기록
        })
    ...
    win_rate = (results_df['strategy_return'] > results_df['market_return']).mean() * 100
    total_trades = results_df['total_trades'].sum()
    avg_liquidations = results_df['total_liquidations'].mean() # 평균 청산 횟수
    ...
    results = {
        ...
        'avg_liquidations_per_window': avg_liquidations # 최종 결과에 추가
    }
